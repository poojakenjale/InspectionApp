...
namespace Inspection.Droid
{
	public class CameraPageRenderer : PageRenderer, TextureView.ISurfaceTextureListener
	{
		...
		public void SetupEventHandlers()
		{
			...
			{
				string Question = App.Current.Properties["cameraClicked"].ToString();
				CreateDirectoryForPictures();				
				AppFile._file = new Java.IO.File(AppFile._dir, String.Format("myPhoto_{0}_{1}.jpg", Guid.NewGuid(), Question));
				var bytes = await TakePhoto();
				System.IO.File.WriteAllBytes(AppFile._file.ToString(), bytes);
				List<AuditData> lstCameraImgs = (List<AuditData>)App.Current.Properties["AuditData"];
				
				switch (Question)
				{
					case "Question1":
						lstCameraImgs[0].ImageUrl = AppFile._file.ToString();
						break;
					case "Question2":
						lstCameraImgs[1].ImageUrl = AppFile._file.ToString();
						break;
					case "Question3":
						lstCameraImgs[2].ImageUrl = AppFile._file.ToString();
						break;
					case "Question4":
						lstCameraImgs[3].ImageUrl = AppFile._file.ToString();
						break;

				}				

				App.Current.Properties["AuditData"] = lstCameraImgs;
				(Element as CameraPage).SetPhotoResult(bytes, liveView.Bitmap.Width, liveView.Bitmap.Height, AppFile._file.ToString());
			};
		...
		{
			AppFile._dir = new Java.IO.File(Android.OS.Environment.GetExternalStoragePublicDirectory(Android.OS.Environment.DirectoryPictures), "Inspection");
